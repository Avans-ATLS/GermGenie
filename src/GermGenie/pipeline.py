...
class EMU:
    ...
    def __init__(self, input_dir: str, output_dir: str, db: str, threads: int = 2, threshold: int = 1) -> None:
        ...
        pd.DataFrame(self.assignment).to_csv(os.path.join(self.output, 'assignments.tsv'), sep='\t', index=False)
    ...
    def run_emu(self, fq: str) -> None:
        ...
        # TODO: Parse assigned/unassigned reads from stdout
    ...
    def species_data(self):
        ...
        species_df.loc[species_df['abundance'] < self.threshold, 'species'] = f'Other species <{self.threshold}%'
        species_df = species_df.groupby(['sample', 'species']).sum().reset_index()
    ...
    def genus_data(self):
        ...
        genus_df.loc[genus_df['abundance'] < self.threshold, 'genus'] = f'Other genera <{self.threshold}%'
        genus_df = genus_df.groupby(['sample', 'genus']).sum().reset_index()
        
        return genus_df
    
    def plot(self, df: pd.DataFrame):
        return px.bar(df, x='sample', y='abundance', color=list(df.columns)[1], title=f'Relative Abundances of {(list(df.columns)[1]).capitalize()}', labels={'sample': 'Sample Name', 'abundance': 'Relative Abundance (%)'})
